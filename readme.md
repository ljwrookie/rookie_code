...
<p align="center">
  ...
  <a href="https://github.com/ljwrookie/rookie_code/blob/main/packages/cli/LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue.svg" alt="License"></a>
...
>
> 本项目主要通过 vibe coding（人类 + LLM 结对）方式编写与迭代。
...
- `packages/website`：项目官网（Vite + React），用于展示与文档页（GitHub Pages 自动部署）
...
# Website
pnpm --filter website dev      # run website locally
pnpm --filter website build    # build website
pnpm --filter website preview  # preview built website
```

## Website

网站代码在 `packages/website`，基于 Vite + React + Tailwind。
...
- push 到 `main` 且变更包含 `packages/website/**` 时，会触发 GitHub Pages 自动部署（见 `.github/workflows/deploy-website.yml`）
...
欢迎提 PR / Issue。建议在 PR 描述中提供：

- 变更动机与行为变化（Breaking changes 需明确）
- 自测方式（命令、截图或复现步骤）
- 相关风险点与回滚方式（如涉及工具权限/命令执行）

鸣谢：

- `GPT-5.4`（文档与代码的结对辅助）
- Claude Code（结对开发工作流参考）
- Codex（代码生成与审阅体验参考）
- Claude 系列模型（对话与推理能力参考）
